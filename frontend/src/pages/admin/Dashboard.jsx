...
function Dashboard() {
  ...
  // Custom Tooltip to show full title
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      const fullTitle = payload[0].payload.fullTitle;
      return (
        <div className="bg-white shadow-md border p-3 rounded-lg text-sm">
          <p className="font-semibold text-gray-800">{fullTitle}</p>
          <p className="text-gray-600">
            {payload[0].dataKey === "lectures"
              ? `Lectures: ${payload[0].value}`
              : `Enrolled: ${payload[0].value}`}
          </p>
        </div>
      );
    }
      ...
      <div className="w-full px-6 py-10 bg-gray-50 space-y-10">
        ...
        <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left space-y-1">
            ...
            <h1 
              className='mt-8 px-[10px] text-center py-[10px] border-2 bg-black border-black text-white rounded-[10px] text-[15px] font-light flex items-center justify-center gap-2 cursor-pointer' 
